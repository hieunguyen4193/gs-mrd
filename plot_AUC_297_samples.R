...
library(stringr)
library(scales)
maindir <- "/media/hieunguyen/HNSD01/src/gs-mrd/output/02_output/model_files/09122024_297_samples"
...
all.features <- setdiff(colnames(maindf), c("SampleID", "SampleID2", "X", "Set", "RUN", "Group_RUN", "Cancer", "True.label", "True_label", "Label"))  
rocobj <- list()

for (feat in all.features){
  if (grepl("prediction_", feat) == FALSE){
...
ggsave(plot = auc.plot, filename = "AUC_297_samples.svg", path = path.to.save.figures, device = "svg", dpi = 300, width = 16, height = 10)
ggsave(plot = auc.plot, filename = "AUC_297_samples.png", path = path.to.save.figures, device = "png", dpi = 300, width = 16, height = 10)
